...
const PRODUCTS = [
  ...
  function addProductToCart(productId) {
    const kiemtra = SHOPPING_CART.findIndex(item => item.id === productId);
    if (kiemtra == -1) {
        SHOPPING_CART.push({ id: productId, quantity: 1 });
    } else {
        SHOPPING_CART[kiemtra].quantity++; // Sửa thành kiemtra
    }
}
  ...
  function removeProductFromCart(productId) {
    const kiemtra = SHOPPING_CART.findIndex(item => item.id === productId);
    if (kiemtra !== -1) {
        if (SHOPPING_CART[kiemtra].quantity > 1) {
            SHOPPING_CART[kiemtra].quantity--;
        } else {
            SHOPPING_CART.splice(kiemtra, 1);
        }
    }
}
